...
# for i in [1,2,3,4,9,10]:
    ...
    # cellpath_obj.all_in_one(flavor = "k-means", num_metacells = num_metacells, resolution = None, n_neighs = 15, num_trajs = num_trajs, prop_insert = 0.30, seed = 0, mode = "exact")
    cellpath_obj.all_in_one(flavor = "hier", num_metacells = num_metacells, resolution = None, n_neighs = 15, num_trajs = num_trajs, prop_insert = 0.70, seed = 0, mode = "exact")
...
# np.save("cellpath/kt_cellpath_cycletree_hier_exact.npy", kt_cellpath)
...
for i in range(1,6):
    ...
    cellpath_obj.all_in_one(flavor = "k-means", num_metacells = num_metacells, resolution = None, n_neighs = 5, num_trajs = num_trajs, insertion = True, prop_insert = 0.30, seed = 0, mode = "fast", pruning = False, scaling = 4, distance_scalar = 0.5)

    visual.first_order_approx_pt(cellpath_obj, basis="pca", trajs = num_trajs, figsize=(10,5), save_as= None)
    kt_cellpath.extend([x for x in bmk.cellpath_kt(cellpath_obj).values()])

np.save("cellpath/kt_cellpath_multicycles_80_fast.npy", kt_cellpath)
...
# np.save("cellpath/kt_cellpath_dyngen_tree_fast.npy", kt_cellpath)
...
# np.save("cellpath/kt_cellpath_velosim_tree_fast.npy", np.array(kt_cellpath))
